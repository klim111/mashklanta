...
const flow: DemoFlow = {
  ...
  steps: [
    ...
    },
    {
      ...
      caption: 'אזור "תכנון הוצאות" הוא כלי תכנון ההון העצמי בתוך הדאשבורד: כל ההוצאות הנלוות של הרכישה — מקדמה, מס רכישה, עו״ד, שיפוץ — עם סכום, מועד וסטטוס.',
      actions: [{ type: 'click', target: 'dash-nav-expenses' }],
      target: 'dash-expenses',
      spotlight: { side: 'over' },
      duration: 6500,
    },
    {
      id: 'expenses-tool',
      title: 'מה שנשמר כאן נכנס ללוח השנה',
      caption: 'מועדי התשלום שמתכננים כאן מופיעים גם בלוח השנה הראשי ובסקירה — כך המעקב אחרי ההוצאות והמשימות הוא אחד. אפשר לעדכן סכומים ולסמן מה כבר שולם.',
      actions: [{ type: 'waitFor', target: 'eq-stepper', optional: true, timeout: 4000 }],
      target: 'eq-stepper',
      spotlight: { side: 'bottom' },
      allowTryIt: true,
      tryItHint: 'הזינו מחיר נכס ומועד יעד, עברו לטבלת ההוצאות ושנו סכומים או סטטוסים — הכול נשאר בדפדפן בלבד.',
    },
    {
      id: 'go-settings',
      title: 'הגדרות ופרופיל',
      caption: 'בהגדרות שומרים את פרטי הלווים, ההון העצמי וההוצאות החודשיות הקבועות — הם נטענים כברירת מחדל לכל משכנתא חדשה ומשפיעים על ההחזר שהבנק יאשר.',
    ...
    {
      id: 'expenses',
      title: 'הוצאות חודשיות קבועות',
      caption: 'נעדכן את ההוצאות החודשיות ל-{{expenses|money}} — כלי ההיתכנות יתחשב בזה בחישוב ההחזר המותר.',
      actions: [
