...
export default function VideoCallModal({ isOpen, onClose, client, advisor }: VideoCallModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Request media access immediately to show preview
  ...
  const initializeDevices = async () => {
    try {
      // First request media access to get device labels (they're empty until permission is granted)
      try {
        await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
      } catch (err) {
        console.log('Initial media request failed (may need user interaction):', err);
      }
  ...
  const requestInitialMediaAccess = async () => {
    try {
      console.log('Requesting initial media access for preview...');
      
      // Request media with current device preferences
      const stream = await requestMediaAccess(
        callState.isVideoOn,
        callState.isAudioOn,
        callState.selectedCameraId || undefined,
        callState.selectedMicrophoneId || undefined
      );
      
      console.log('Initial media access granted, setting up preview');
      ...
      if (localVideoRef.current) {
        ...
        try {
          ...
          // Still set permission as granted, user may need to interact
          setMediaPermissionGranted(true);
        }
      }
    } catch (error) {
  ...
  // Initialize guard to prevent race conditions
  const isInitializingRef = useRef(false);
  const initializationMutexRef = useRef<Promise<void> | null>(null);

  const initializeWebRTC = async () => {
    // Prevent multiple initializations with mutex pattern
    if (isInitializingRef.current) {
      console.log('Advisor WebRTC initialization already in progress, waiting...');
      if (initializationMutexRef.current) {
        await initializationMutexRef.current;
      }
      return;
    }

    if (peerConnectionRef.current) {
      console.log('Advisor WebRTC already initialized, skipping...');
      return;
    }

    // Set initialization flag and create mutex promise
    isInitializingRef.current = true;
    initializationMutexRef.current = (async () => {
      try {
      ...
      peerConnectionRef.current.onicecandidate = (event) => {
        ...
        initializationMutexRef.current = null;
      }
    })();
  ...
  const switchCamera = async (cameraId: string) => {
    ...
    try {
      ...
      console.log('Camera switched successfully:', cameraId);
    } catch (error) {
  ...
  const handlePermissionGranted = async () => {
    ...
    setMediaPermissionGranted(true);
    setShowPermissionCheck(false);
    // Request media access now that permission is granted
    await requestInitialMediaAccess();
  };
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="flex-1 relative bg-gray-900 rounded-lg m-4">
                {showPermissionCheck ? (
                  ...
                  <>
                    {/* Local Video Preview - Always visible when permission granted */}
                    {mediaPermissionGranted && localStreamRef.current && (
              ...
              {/* Device Selection - Always visible when permission granted */}
              {mediaPermissionGranted && callState.availableCameras.length > 0 && (
