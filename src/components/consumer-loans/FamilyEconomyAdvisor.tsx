...
/**
 * הפנייה ליועץ כלכלת המשפחה של משכלנתא, מתוך כלי ההלוואות.
 *
 * הכלי מראה ללקוח את המצב; היועץ הוא מי שהופך אותו לתוכנית — איחוד או פריסה
 * של ההלוואות היקרות, שימוש נכון בכסף שיש לו, ותשואה טובה יותר על החיסכון.
 * הפנייה נשלחת כבקשת ליווי לצד היועצים, עם הסבר ועם תמונת התיק שהלקוח הזין,
 * כדי שהיועץ יחזור אליו עם משהו ביד. לקוח שאינו רשום מזין שם וטלפון, והמייל
 * אצלו הוא רשות.
 */
...
export interface AdvisorLeadContext {
  /** תמונת התיק שהלקוח הזין, נשלחת ליועץ כהערה */
  summary?: string;
}
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const notes = [
        ...
        context?.summary ? `תמונת התיק שהלקוח הזין בכלי: ${context.summary}` : null,
        note.trim() || null,
      ]
        .filter(Boolean)
        .join('\n');
        ...
        {sent ? (
          ...
          >
            ...
            <DialogDescription className="mt-3 text-sm leading-relaxed text-slate-500">
              הבקשה מופיעה אצל יועצי משכלנתא כבקשת ליווי, יחד עם תמונת ההלוואות שהזנתם כאן — כך
              שהשיחה מתחילה מהנתונים שלכם ולא מאפס. בינתיים אפשר להמשיך לעבוד בכלי.
            </DialogDescription>
            <button
              type="button"
            ...
            <div className="mb-4 flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <DialogDescription className="mt-1 text-[13px] leading-relaxed text-slate-500">
                  השאירו שם וטלפון, ויועץ יחזור אליכם עם קריאה ראשונה של ההלוואות שהזנתם כאן — מה
                  יקר, מה כדאי לאחד או לסגור, ואיך לנצל טוב יותר את הכסף שברשותכם.
                </DialogDescription>
              </div>
            </div>
            ...
            >
              ...
              <div className="space-y-3 p-5">
                <ul className="m-0 list-none space-y-2 p-0">
                  {ADVISOR_VALUE_POINTS.map((point) => {
