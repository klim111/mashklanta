      ...
      return shell(
        <div className="relative">
          <NumericInput
            ...
            readOnly={readOnly}
            disabled={readOnly}
            // Shekel amounts and counts are whole numbers, so they get live
            // thousands separators; percentages keep the decimal behaviour.
            integer={field.kind === 'integer' || field.kind === 'money'}
            max={field.max}
            className={`${inputClass({ error, readOnly })} ${suffix ? 'pl-10' : ''}`}
          />
          {suffix && (
