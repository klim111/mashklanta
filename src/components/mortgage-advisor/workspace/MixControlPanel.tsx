        ...
        <Field icon={Percent} label="ריבית שנתית">
          <div className="flex items-center gap-2">
            <Slider
              dir="ltr"
              className="flex-1"
              value={[Math.min(12, Math.max(0.1, track.interestRate))]}
              onValueChange={([value]) => patchWithName({ interestRate: Number(value.toFixed(2)) })}
              min={0.1}
              max={12}
              step={0.05}
            />
            <NumericInput
              className="h-7 w-16 shrink-0 rounded-md border border-slate-200 bg-slate-50 px-1 text-center text-[12px] font-bold text-slate-700 shadow-sm"
              value={track.interestRate}
              onChange={(rate) => patchWithName({ interestRate: rate ?? 0 })}
            />
          </div>
        </Field>
