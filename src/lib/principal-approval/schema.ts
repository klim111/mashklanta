...
export interface FieldDef extends ValidatableField {
  ...
  span?: 1 | 2 | 3;
  /**
   * Stored encrypted (src/lib/crypto.ts), following the same policy as the
   * encrypted financial columns on `Client`. Encrypted values cannot be
   * filtered or aggregated in SQL — only read back per row.
   */
  sensitive?: boolean;
  suffix?: string;
  min?: number;
...
export const INCOME_FIELDS: FieldDef[] = [
  ...
  { key: 'monthlyAmount', kind: 'money', label: 'סכום חודשי נטו', required: true, suffix: '₪', sensitive: true },
  {
    key: 'employerName',
    kind: 'text',
...
export const PREV_EMPLOYMENT_FIELDS: FieldDef[] = [
  ...
  { key: 'monthlyAmount', kind: 'money', label: 'שכר חודשי נטו', required: true, suffix: '₪', sensitive: true },
...
export const BANK_ACCOUNT_FIELDS: FieldDef[] = [
  ...
  { key: 'accountNumber', kind: 'bankAccount', label: 'מספר חשבון', required: true, sensitive: true },
  {
    key: 'ownerIds',
    kind: 'multiselect',
    label: 'בעלי החשבון',
...
export const FUNDING_SOURCE_FIELDS: FieldDef[] = [
  ...
  { key: 'amount', kind: 'money', label: 'סכום', required: true, suffix: '₪', sensitive: true },
...
/** Is this field stored encrypted at rest? */
export function isSensitiveField(entityType: EntityType, fieldKey: string): boolean {
  return Boolean(getFieldDef(entityType, fieldKey)?.sensitive);
}
