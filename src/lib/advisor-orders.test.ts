...
describe('quoteOrder — קיזוז הגישה לפלטפורמה', () => {
  it('מה ששולם על הפלטפורמה יורד מהמחיר, ולא מתחת לאפס', () => {
    expect(quoteOrder(['SIGNING'], 100).total).toBe(ADVISOR_STAGE_PRICE.SIGNING - 100);
    expect(quoteOrder(['SIGNING'], 1_000_000).total).toBe(0);
    expect(quoteOrder(['SIGNING'], 100).platformCredit).toBe(100);
    expect(quoteOrder([], 100).total).toBe(0);
  });

  it('צירוף חלקי שיקר מליווי מלא מחויב במחיר הליווי המלא', () => {
    const four = quoteOrder(PLAN_STAGES.slice(0, 4) as PlanStageId[]);
    expect(four.total).toBeLessThanOrEqual(quoteOrder(PLAN_STAGES).total);
  });
});
