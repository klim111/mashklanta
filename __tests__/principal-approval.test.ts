...
import { profileKeyFor, valuesDiffer } from '@/lib/principal-approval/profile';
...
describe('profile sync', () => {
  it('maps profile keys for the primary borrower only', () => {
    expect(profileKeyFor('borrower', 'firstName', true)).toBe('firstName');
    expect(profileKeyFor('borrower', 'firstName', false)).toBeNull();
    expect(profileKeyFor('case', 'totalEquity', true)).toBe('totalEquity');
    expect(profileKeyFor('income', 'employerName', true)).toBeNull();
  });
